...
for c in SUBCAT_LIST:
    ...
    DATAF_SHAP.insert(10, "PREDICTED",DATAF.SALES_VOLUME_PREDICTED.values)
    res = list((Counter(all_features)-Counter(sales_input_col)).elements())
    DATAF_SHAP[res] = 0
    DATAF_SHAP['underlying_growth'] = DATAF_SHAP[['YEAR','MONTH','SEASONALITY_INDEX_SALES','SALES_TREND_CAL']].sum(axis=1)
    DATAF_SHAP['consumer_behaviour'] = DATAF_SHAP[['AVG(TRAFFIC_WEIGHT)', 'SUM(NEW_CASES)', 'SUM(NEW_DEATHS)', 'SUM(CUMU_CASES)',
                                           'SUM(CUMU_DEATHS)']].sum(axis=1)
    DATAF_SHAP['weather'] = DATAF_SHAP[['AVG(MAX_TEMP_CELSIUS)',
                                'AVG(MIN_TEMP_CELSIUS)', 'AVG(AVG_TEMP_CELSIUS)', 'AVG(HUMID_PCT)',
                                'AVG(FEELS_LIKE_CELSIUS)', 'AVG(AVG_WIND_MPH)', 'AVG(PRECIP_MM)','AVG(SNOW_CM)']].sum(axis = 1)
    DATAF_SHAP['marketing_mix'] = DATAF_SHAP.PRICE_PER_VOL + DATAF_SHAP.TDP
    DATAF_SHAP['macro_economic'] = DATAF_SHAP[['AVG(CONSUMER_PRICE_INDEX)', 'AVG(PERSONAL_DISPOSABLE_INCOME_REAL_LCU)',
                                       'AVG(GDP_NOMINAL_LCU)', 'AVG(GDP_REAL_LCU)', 'AVG(RETAIL_PRICES_INDEX)',
                                       'AVG(SHARE_PRICE_INDEX)', 'AVG(UNEMP_RATE)']].sum(axis=1)
...
shap_file = 'CHN_'+str(DATAF_ALL_HIST_PROJ.SUBCAT_NAME.unique()[0])+'shap'+str(SCENARIO_NO)+'.csv'
